#!/usr/bin/php
<?php
$plcname = "Ww-INPUTWEST";
require "template.php";
/// Waste Water System, Main City Inlet EAST SIDE!
/// 
/// PORT 0000 - Input Rate at kGPM
//////////////////////////////////////////
/// MEMCACHE KEYS
/////////////////////////////////////////
...
function process_logic($input)
{
    ...
    if ($res !== false) {
        ...
        if ($res["act"] === "1") {
            ...
            switch ($res["port"]) {
                case "0000":
                        ...
                        $lakelevel = $m->get('WW-INPUTWEST');
                        if (empty($lakelevel)) {
                            //empty lake, lets refill it!
                            $m->set('WW-INPUTWEST', 20);
                            $lakelevel = 20;
                            logme('InputWest Empty, Resetting to 20kGPM');
                        }
                        logme('Sending WW-INPUTWEST of ' . $lakelevel);
        ...
        if ($res["act"] === "0") {
            switch ($res["port"]) {
                    case "0000":
                        simcrash("Tried to write to read only port for input west!");
                    break;
                    default:
                        logme('Sending Unused Port ' . $res["port"]. " false data");

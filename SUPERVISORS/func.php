...
function plc_read($servername, $port){
    global $servers;
    logme("Contacting: " . $servername);
    $fp = fsockopen($servers[$servername][0], $servers[$servername][1], $errno, $errstr, 30);
    if (!$fp) {
        logme("=== ERRTCP: $errstr");
        return false;
    } else {
        $data = data_writer("1", $port, 0, "1101", "1101");
        fwrite($fp, $data);
        while (!feof($fp)) {
...
function plc_set($servername, $port, $value){
    ...
    if (!$fp) {
        ...
        $data = data_writer("0", $port, $value, "1011", "1101");
        fwrite($fp, $data);
        while (!feof($fp)) {
            $data = fgets($fp, 6);
        }
